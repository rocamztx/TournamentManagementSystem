    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ Pontuação salva! Atualizando painel...'),
  ...
  Future<void> _confirmarEZerarPontuacao() async {
    ...
    bool? confirmar = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Isso vai apagar TODO o histórico de notas da $_nomeEquipeSelecionada. Deseja continuar?',
        ),
        actions: [
          TextButton(
    ...
    if (confirmar == true) {
      try {
        await _apiService.zerarPontuacao(_equipeSelecionadaId!);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('🗑️ Pontuação zerada com sucesso!'),
                  ...
                  child: DropdownButton<int>(
                    ...
                    onChanged: (novoId) {
                      setState(() {
                        ...
                        _nomeEquipeSelecionada = _equipesBanco
                            .firstWhere((e) => e.id == novoId)
                            .nomeDaEquipe;
                      });
                    },
                  ),
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                '$_pontuacaoCalculada pontos (total)',
                style: const TextStyle(
                  fontSize: 40,
                  color: Colors.white,
  ...
  Widget _construirBotaoRound(int round, String texto) {
    ...
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        ...
        elevation: 0,
        side: BorderSide(
          color: selecionado ? AppColors.techBlue : Colors.grey.shade300,
        ),
        padding: const EdgeInsets.symmetric(vertical: 20),
